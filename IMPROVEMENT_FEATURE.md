...
Tower Management System (External) → CSV export (tower_lat, lon, id, name) 
    → POST /api/v1/missions/locations/upload (CSV import)
    ...
    → Scan result linked back to mission planner (1-to-1)
    → Broadcast progress via WebSocket
    → Mission status: IDLE→STARTING→RUNNING→COMPLETED|STOPPED|FAILED
    → On app restart: RUNNING mission becomes STOPPED (user manual recovery)
```
...
|-------|--------|-------------|
| `scan_sessions` | `app` | Scan header (tty_port, latitude, longitude, created_at) — **add `mission_planner_id` (nullable)** |
...
#### A. `mission_locations` — Persistent tower master data
Persistent store of imported tower coordinates, reusable across missions.

```sql
CREATE TABLE app.mission_locations (
    id SERIAL PRIMARY KEY,
    cellular_tower_id VARCHAR(100) UNIQUE NOT NULL,      -- External TMS ID
    cellular_tower_name VARCHAR(255),
    latitude FLOAT NOT NULL CHECK (latitude BETWEEN -90 AND 90),
    longitude FLOAT NOT NULL CHECK (longitude BETWEEN -180 AND 180),
    source VARCHAR(50) DEFAULT 'csv_import',
    upload_batch_id VARCHAR(36),                          -- Group csv uploads
    created_at TIMESTAMPTZ DEFAULT NOW(),
    updated_at TIMESTAMPTZ DEFAULT NOW()
);

CREATE UNIQUE INDEX idx_mission_location_tower_id ON app.mission_locations(cellular_tower_id);
CREATE INDEX idx_mission_location_coord ON app.mission_locations(latitude, longitude);
```

#### B. `missions` — Mission header + state machine
...
CREATE TABLE app.missions (
    ...
    start_location_id INTEGER REFERENCES app.mission_locations(id) ON DELETE SET NULL,
    current_location_id INTEGER REFERENCES app.mission_locations(id) ON DELETE SET NULL,
...
#### C. `mission_planners` — Ordered visit sequence (1-to-1 link to scan_session)

```sql
CREATE TABLE app.mission_planners (
    id SERIAL PRIMARY KEY,
    mission_id INTEGER NOT NULL REFERENCES app.missions(id) ON DELETE CASCADE,
    mission_location_id INTEGER NOT NULL REFERENCES app.mission_locations(id) ON DELETE RESTRICT,
    ...
    CONSTRAINT unique_sequence_mission UNIQUE (mission_id, sequence_order)
);

CREATE INDEX idx_mission_planners_mission ON app.mission_planners(mission_id);
CREATE INDEX idx_mission_planners_sequence ON app.mission_planners(sequence_order);
CREATE INDEX idx_mission_planners_status ON app.mission_planners(status);
CREATE INDEX idx_mission_planners_scan_session ON app.mission_planners(scan_session_id);
```

**Note:** `scan_session_id` in `mission_planners` enforces **1-to-1 with scan_sessions**. Since it's `UNIQUE`, each scan session can be linked to at most one mission planner. Conversely, `mission_planner_id` added to `scan_sessions` is optional (for backward compatibility).
...
|--------|----------|-------------|
| `POST` | `/locations/upload` | Multipart/form-data CSV upload (file), returns batch metadata |
| `GET`  | `/locations` | Paginated list with search/filter by tower_id/name |
| `GET`  | `/locations/{id}` | Single location detail |
| `DELETE` | `/locations/{id}` | Delete single location (blocked if referenced by active mission) |
| `POST` | `/locations/bulk-delete` | Delete by upload_batch_id (cleanup) |
...
| `GET`  | `/{id}` | Get mission detail + planners (nested route) |
...
| `POST` | `/{id}/route/skip` | Mark specific planner as SKIPPED |

**Auto-planner algorithm:** Nearest-Neighbor from start_location, then 2-opt local optimization for refinement. Output updates `mission_planners.sequence_order` and computes `distance_from_previous_meters`, `bearing_from_previous_degrees`.
...
- `RUNNING` → `COMPLETED`: all planners reached
...
class MissionExecutor:
    def __init__(self):
        ...
        # On app load, restore all RUNNING missions as STOPPED
        async def restore_running():
            db = get_db()
            for m in db.query(Mission).where(Mission.status == 'RUNNING').all():
                m.status = 'STOPPED'
                m.stopped_at = now()
            db.commit()
    ...
    async def run_mission(self, mission_id: int):
        ...
        try:
            ...
            while mission.status == 'RUNNING':
                ...
                # Find next pending planner
                planner = self._get_next_pending(mission.id)
                if not planner:
                ...
                lat2, lon2 = planner.location.latitude, planner.location.longitude
                ...
                if dist <= mission.radius_meters:
                    ...
                    try:
                        scan_result = self._trigger_scan(port, loc.latitude, loc.longitude, mission.id, planner.id)
                        planner.scan_session_id = scan_result.id
                        planner.status = 'VISITED'
                        planner.actual_visit_time = datetime.now(tz=UTC)
                        planner.visited_at = datetime.now(tz=UTC)
                        mission.visited_locations += 1
                        mission.current_location_id = planner.mission_location_id
    ...
    def _trigger_scan(self, port: str, lat: float, lon: float, mission_id: int, planner_id: int) -> ScanResult:
        """Execute scan and save result with reference."""
        db = SessionLocal()
        try:
            # Call scan_service with optional mission reference
            service = ScanService(db=db)
            session = service.execute_scan_with_mission_ref(
                port=port,
                latitude=lat,
                longitude=lon,
                mission_planner_id=planner_id,
...
Modify `ScanService.execute_scan()` to accept optional `mission_planner_id: int | None = None`. This value gets stored in the newly added `mission_planner_id` column on `scan_sessions`. The modified signature must remain backward-compatible (keyword-only with default).
...
| User modifies mission location after planning | Invalidate planners (set to need regenerate) on PATCH |
| Manual reorder conflicts with auto-plan | Manual overrides take precedence; auto-plan clears any previous plan first |
| Mission planner scan_session_id already taken | UNIQUE constraint enforces 1-to-1; duplicate fails gracefully |
| App restarts during RUNNING mission | Restore → STOPPED on startup; user must manually restart |
...
|-------|-------|--------------|
| **Phase 1** | Data Model | Add tables `mission_locations`, `missions`, `mission_planners`; add `mission_planner_id` to `scan_sessions`; migrate via Alembic |
| **Phase 2** | Location CRUD | Backend models, repositories, schemas, endpoints `/locations/*`, CSV upload endpoint, UPSERT logic |
...
| **Phase 5** | Scanner Integration | Modify `ScanService.execute_scan()` to accept `mission_planner_id`; extend `ScanSessionResponse` to include `mission_planner_id` |
...
✅ Added `mission_planner_id` to `scan_sessions` is **nullable**, so old records stay valid  
