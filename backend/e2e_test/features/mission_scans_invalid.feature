Feature: Mission Scans — List for Non-Existent Mission (P1)
  As the system I must return 404 when a caller requests scans
  for a mission that does not exist so callers can distinguish
  missing missions from empty scan lists.

  Background:
    Given the backend is running on port 8001

  Scenario: S37 List scans for non-existent mission returns 404
    When I list mission scans with mission id 999999
