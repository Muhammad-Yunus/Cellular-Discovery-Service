...
@when('I get status for mission id {mission_id:d}')
def get_mission_status(context, mission_id):
    r = httpx.get(f"{BASE_URL}/api/v1/missions/{mission_id}/status")
    context.mission_status_status = r.status_code
    try:
        context.mission_status_body = r.json()
    except Exception:
        context.mission_status_body = {"raw": r.text}


@then('the mission status response status is {code:d}')
def assert_mission_status_response_status(context, code):
    assert context.mission_status_status == code, (
        f"Mission status response returned {context.mission_status_status}, expected {code}: {context.mission_status_body}"
    )


@then('the mission status response detail mentions "{expected_text}"')
def assert_mission_status_response_detail_mentions(context, expected_text):
    detail = context.mission_status_body.get("detail", "")
    assert expected_text.lower() in detail.lower(), (
        f"Expected mission status response detail to mention '{expected_text}', got '{detail}'"
    )


@when('I plan route for mission id {mission_id:d}')
def plan_mission_route(context, mission_id):
    r = httpx.post(f"{BASE_URL}/api/v1/missions/{mission_id}/plan", json={})
    context.mission_plan_status = r.status_code
    try:
        context.mission_plan_body = r.json()
    except Exception:
        context.mission_plan_body = {"raw": r.text}


@then('the mission plan response status is {code:d}')
def assert_mission_plan_response_status(context, code):
    assert context.mission_plan_status == code, (
        f"Mission plan response returned {context.mission_plan_status}, expected {code}: {context.mission_plan_body}"
    )


@then('the mission plan response detail mentions "{expected_text}"')
def assert_mission_plan_response_detail_mentions(context, expected_text):
    detail = context.mission_plan_body.get("detail", "")
    assert expected_text.lower() in detail.lower(), (
        f"Expected mission plan response detail to mention '{expected_text}', got '{detail}'"
    )
...
@when('I delete mission location {location_id:d} for mission id {mission_id:d}')
def delete_mission_location(context, location_id, mission_id):
    r = httpx.delete(f"{BASE_URL}/api/v1/missions/{mission_id}/locations/{location_id}")
    context.mission_location_delete_status = r.status_code
    try:
        context.mission_location_delete_body = r.json()
    except Exception:
        context.mission_location_delete_body = {"raw": r.text}


@then('the mission location delete status is {code:d}')
def assert_mission_location_delete_status(context, code):
    assert context.mission_location_delete_status == code, (
        f"Mission location delete returned {context.mission_location_delete_status}, expected {code}: {context.mission_location_delete_body}"
    )


@then('the mission location delete detail mentions "{expected_text}"')
def assert_mission_location_delete_detail_mentions(context, expected_text):
    detail = context.mission_location_delete_body.get("detail", "")
    assert expected_text.lower() in detail.lower(), (
        f"Expected mission location delete detail to mention '{expected_text}', got '{detail}'"
    )
...
@when('I list mission scans with mission id {mission_id:d}')
def list_mission_scans(context, mission_id):
    r = httpx.get(f"{BASE_URL}/api/v1/missions/{mission_id}/scans")
    context.mission_scan_list_status = r.status_code
    try:
        context.mission_scan_list_body = r.json()
    except Exception:
